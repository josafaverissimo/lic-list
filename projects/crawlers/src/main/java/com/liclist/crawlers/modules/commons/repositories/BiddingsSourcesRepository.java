...
public class BiddingsSourcesRepository {
    ...
    public Optional<BiddingSource> getByName(String name) {
        return sessionFactory.fromTransaction(session -> {
            ...
            BiddingSource data = session.createSelectionQuery(query, BiddingSource.class)
    ...
    public BiddingSource create(BiddingSource biddingSource) {
        return sessionFactory.fromTransaction(session -> {
            ...
            logger.info(String.format("Bidding source #(name=%s) has been created", biddingSource.getName()));
